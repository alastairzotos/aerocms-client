...
export type CallStatus = null | 'fetching' | 'success' | 'failure';
...
const performFetch = <T = any>(req: IRequest): Promise<IResponse<T>> =>
    new Promise(async (resolve, reject) => {
        try {
            ...
            const token: string = JSON.parse(JSON.parse(localStorage.getItem('persist:admin-auth')).loginToken);
            ...
            const response = await window.fetch(config.domain + req.url, {
                method: req.method,
                headers: {
                    ...contentHeaders,
                    ...authHeaders
